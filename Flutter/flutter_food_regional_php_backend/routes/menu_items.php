...
function handleMenuItemRoutes($method, $path) {
    ...
    if ($method === 'POST' && $path === '') {
        ...
        try {
            $stmt = $db->prepare('INSERT INTO menu_items (id, restaurant_id, name, description, price, image_url) VALUES (?, ?, ?, ?, ?, ?)');
            $stmt->execute([$id, $restaurantId, $name, $description, $price, $imageUrl]);
    ...
    if ($method === 'PUT' && preg_match('/^\/([a-f0-9-]+)$/', $path, $matches)) {
        ...
        try {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                http_response_code(404);
                echo json_encode(['error' => 'Menu item not found or no changes made']);
            }
        } catch (PDOException $e) {
            http_response_code(500);
            echo json_encode(['error' => 'Database error: ' . $e->getMessage()]);
