...
class _ManageOrdersScreenState extends State<ManageOrdersScreen> {
  ...
  List<Order> get _orders => MockRepository.getOrdersBySeller(_currentSellerId);
              ...
              itemBuilder: (context, index) {
                ...
                return _OrderCard(
                  ...
                  onStatusChanged: (newStatus) {
                    setState(() {
                      MockRepository.updateOrderStatus(order.id, newStatus);
                    });
                  },
                );
              },
          ...
          ...order.items.map((item) => Padding(
            ...
            child: Row(
              children: [
                ClipRRect(
                  borderRadius: BorderRadius.circular(8),
                  child: item.saree.imageUrls.first.startsWith('http')
                      ? Image.network(item.saree.imageUrls.first, width: 40, height: 40, fit: BoxFit.cover)
                      : Image.asset(item.saree.imageUrls.first, width: 40, height: 40, fit: BoxFit.cover),
                ),
                ...
                Text('₹${item.price}', style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.bold, color: AppColors.primary)),
              ],
            ),
          )),
          const Divider(height: 24),
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text('₹${order.totalAmount}', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.primary)),
                ],
              ),
